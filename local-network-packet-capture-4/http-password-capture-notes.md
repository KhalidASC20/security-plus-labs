Capturing unprotected HTTP login credentials
Date: 08/11/26
...
5. Stopped the capture and applied the display filter http to isolate application-layer HTTP traffic 
...
This confirmed, using a real external website rather than a local test page, that plain-HTTP login forms transmit credentials with zero confidentiality protection

This is a real-world demonstration of the confidentiality risk posed by unencrypted protocols, and specifically why HTTPS exists. A site handling login credentials over plain HTTP offers no protection in transit — the data is exposed identically to how it was typed, to anyone positioned to observe the traffic. This is not a theoretical risk: credential harvesting over unsecured networks (public Wi-Fi, compromised routers, or on-path attacks) is one of the most common and lowest-effort ways real attackers obtain valid login credentials.
